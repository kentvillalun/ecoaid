...
## Completed (continued — Sep 5–12, 2026 update)

- **Super Admin module built end-to-end** — the item previously tracked under "In Progress" is done, including the barangay-facing conditional rendering that was called out as a separate follow-up.
  - **Login**: `POST /auth/admin/login` (`superAdminLogin`) / `POST /auth/admin/logout` (`superAdminLogout`) in `auth.controller.js`; issues an `admin_token` httpOnly cookie (7-day expiry), rejects any non-`SUPER_ADMIN` role or inactive account; `authenticateSuperAdmin` middleware (`authMiddleware.js`) verifies the JWT, checks the `BlackListedToken` table, and requires `role === "SUPER_ADMIN"`. `frontend/src/app/(auth)/admin/login/page.jsx` — separate "Super Admin Portal" login page; stores `ecoaidAdminSession` in `localStorage` on success and redirects `/admin-dashboard` on mount if already present (same not-JWT-verified presence-check pattern used by barangay login).
  - **`(admin)` route group** — `frontend/src/app/(admin)/layout.jsx` wraps pages in `AdminSidebar` + `AdminDrawerContext`, mirroring the barangay layout's drawer pattern.
  - **`/admin-dashboard`** — `GET /admin/onboarding-status` (`admin-dashboard.controller.js`'s `getOnboardingStatus`) returns every barangay with a computed `isIncomplete` flag (missing `CAPTAIN`/`SECRETARY`/`TREASURER` staff role, or zero sitios); dashboard page shows "Barangays onboarded" vs. "Need attention" counts plus an `IncompleteSetupTable`/`IncompleteSetupCard` listing exactly which roles/sitios are missing per barangay.
  - **`/barangay-accounts`** (list, `/register`, `/[id]`, `/[id]/edit`) — full barangay CRUD via `barangay-accounts.controller.js`/`route.js`, mounted at `/admin`: `registerBarangay` (name, municipality, province, zipCode, contactNumber, `themeAccent`, and the four `has___` feature flags; rejects duplicate zip code or contact number), `editBarangay` (partial update, same uniqueness checks excluding self), `getBarangayDetails`, `registerStaffAccount` (creates a `User` scoped to `barangayId` with a role from `BARANGAY_ROLES`, checks username/phone uniqueness), `getStaffAccountsByBarangay`, `createSitio`/`getSitios`/`editSitios` (per-barangay, unique per `(barangayId, name)`). Detail page shows basic info, sitios (`SitiosSection`), assigned theme preset, enabled-module tags, and a staff table with an `AddStaffModal`.
  - **Barangay `has___` feature flags are now actually enforced on the barangay-facing frontend** — `frontend/src/lib/getBarangayModules.js` + a `MODULE_FLAG_MAP` in `Sidebar.jsx` filter `topLevelItems`/`managementItems`/`communicationItems` by both `ROLE_MATRIX` (role) and the barangay's own flags (`passesModuleCheck`), so a barangay without e.g. `hasLeaderboard` doesn't see that nav item regardless of staff role. Also consumed by `program-funds/page.jsx` and the resident-side `home`/`standings` pages.
  - All `/admin` routes protected by `authenticateSuperAdmin` only (no per-route role list needed — there's only one admin role).

- **Cross-tenant IDOR fixed** — `updateStatus`/`getRequest` (`pickup-request.controller.js`) and `updateProgram` (`redemption.controller.js`) previously looked up records by `id` alone; a barangay admin with a valid token could view/approve/reject/complete another barangay's pickup requests, or edit another barangay's redemption program, just by knowing/guessing the record id. Fixed by adding `barangayId` (from `req.user`) into every `where` clause; a mismatched id now resolves to "not found" instead of leaking or mutating another tenant's row. Announcement queries scoped the same way in the same commit.

- **Claude-powered recyclable image classification (resident capture flow) shipped end-to-end.**
  - **Backend**: `POST /pickup-requests/classify` (`image-recognition.controller.js`'s `classifyRecyclable`) — accepts a base64 `image` + `mimeType`, sends it to Claude (`claude-sonnet-5` via `@anthropic-ai/sdk`) alongside a prompt built by `backend/src/utils/coreMaterials.js`'s `buildClassificationPrompt()`, strips any markdown code-fence wrapping from the response, and returns the parsed JSON classification. Protected by `authenticateResident` + `requireRoles(["RESIDENT"])`, plus a dedicated `classifyLimiter` (`express-rate-limit`, 5 requests/min) since each call is a paid Claude API request. `express.json({ limit: "10mb" })` added in `server.js` to accommodate the base64 payload.
  - **`CORE_MATERIALS`** (`coreMaterials.js`) — a fixed catalog of material names grouped by category (Metals/Papers/Plastics/Glass) with a `defaultUnit`, used only to constrain the prompt (Claude must pick a name from this exact list); the frontend then name-matches the returned `materialCategory`/`material` strings against the live `Category`/`Material` DB records to get real ids. The prompt also: distinguishes piece-counted vs. weight-based materials, combines multiple different materials in one photo into a single `isAssorted: true` result with one combined kg estimate, and returns `material: null`/`estimatedValue: 0` with an explanatory `notes` string when the photo is unclear or unrecognizable.
  - **Frontend (`capture/page.jsx`)**: "Analyze Photo" button runs `analyzePhoto()` — compresses the captured image client-side via `browser-image-compression` (`maxSizeMB: 1`, `maxWidthOrHeight: 1024`) before both classification and the later Cloudinary upload (the compressed file is cached in state so it isn't recompressed at submit time), base64-encodes it, calls `/api/pickup-requests/classify`, then auto-fills the form: matches the returned category/material names to live ids (via a `pendingMaterialName` state watched by an effect on `materialData`), sets `estimatedValue`/`notes`/`estimatedUnit` (locking the unit select when the matched material's `defaultUnit` is `PIECE`), and sets the assorted toggle. If classification fails, times out, or the resident opts out, a "Fill in manually" link reveals the same form empty for manual entry.
  - **Leave-page guard** — if a photo has been captured but the request not yet submitted, tapping back opens a "Discard this request?" confirmation modal instead of navigating away immediately.
  - **Bug fixes bundled into this feature**: notes are now correctly forwarded from the classification result into the form (previously dropped); Cloudinary upload is called from `onSubmit` rather than a separate effect, removing a stale-state bug; notes input changed from a single-line field to a `<textarea>`; barangay-side `RequestCard` now slices/character-limits displayed notes; `estimatedValue` input given `step="0.01"`.

- **Deployment: backend moved from Railway to Render.** `frontend/next.config.mjs`'s API base URL now points at `https://ecoaid-ydrs.onrender.com` (Railway URL kept only as a commented-out fallback); `@prisma/adapter-pg` moved from devDependencies to dependencies since it's required at runtime on the new host.

- **Smaller fixes bundled across this period**: barangay PWA manifest moved to `public/` with metadata defined in the barangay layout (fixes install/display bugs), `apple-web-app` metadata and manifest `scope` added; barangay login and admin login both gained a mount-time session check that redirects away from the login form if a session already exists; barangay logout clears the `localStorage` session item; dashboard/summary cards now fall back to a default value instead of a skeleton while loading; `Empty` component import bug fixed in `JunkshopsSection`; arrow icon removed from summary cards, oversized empty-state text fixed, missing empty states added to filtered tables; resident card and barangay dashboard card layout bugs fixed; a sample resident row seeded in dev data since the OTP SMS provider isn't wired up for real sends yet; material tag "dot" removed from the resident home screen; attempted fix for a scroll bug on the resident history page.

**Known limitation, not yet fixed**: role-based access (`ROLE_MATRIX`) and barangay feature-flag access (`has___`) are two separate axes, and only the role axis is enforced server-side. `proxy.js` decodes the JWT and checks the route against `ROLE_MATRIX` on every request — a role without access to a route is redirected to `/403` regardless of direct-URL access. But `proxy.js` never fetches the barangay's `has___` flags at all; that check only happens client-side, in `Sidebar.jsx`'s `passesModuleCheck` (hides the nav link) and in the handful of pages that explicitly call `getBarangayModules` themselves (`program-funds`, resident `home`/`standings`). So a staff member whose *role* is allowed into a route (per `ROLE_MATRIX`) but whose *barangay* has that module's flag disabled can still reach it directly by URL — e.g. a CAPTAIN at a barangay with `hasRedemptionManagement: false` is not blocked from `/redemption` itself, only from seeing its Sidebar link.

---

## In Progress

- Nothing currently tracked as in-progress as of this update — see Next Steps below for what's queued next.

---

## Next Steps (priority order)

1. Audit barangay `has___` feature-flag enforcement beyond Sidebar filtering — add middleware-level (or at least page-level) gating so a disabled module can't be reached by direct URL, matching how `ROLE_MATRIX` is already enforced in `proxy.js`
2. Technical debt batch:
   ...
   - Rate limiting and other backend security/hardening — still only exists on the `/pickup-requests/classify` endpoint (5 req/min, added because it's a paid Claude API call); every other endpoint remains unlimited
...
- Super Admin (`SUPER_ADMIN` role) is a separate portal from barangay staff — its own login page/cookie (`admin_token`)/middleware (`authenticateSuperAdmin`), mounted under `/admin` on the backend and the `(admin)` route group on the frontend; it is not part of `ROLE_MATRIX` (that matrix is barangay-staff-only)
- The AI classification prompt's material list (`CORE_MATERIALS` in `backend/src/utils/coreMaterials.js`) is a separate, hand-maintained catalog from the live `Material`/`Category` DB tables — adding a new material via Settings does not make Claude able to detect it; `CORE_MATERIALS` must be updated too, and its names must exactly match the DB `Material.name` values for the frontend's name-matching to work
- Backend is deployed on Render (`ecoaid-ydrs.onrender.com`), not Railway — the Railway URL is kept commented out in `frontend/next.config.mjs` as a historical fallback only
